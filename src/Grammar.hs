...
happyExpList = Happy_Data_Array.listArray (0,19) ([33008,15360,0,32,960,61472,0,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseCalc","Exp","print","append","duplicate","'('","')'","num","%eof"]
        bit_start = st * 11
        bit_end = (st + 1) * 11
...
happyReduction_2 (HappyAbsSyn4  happy_var_3)
	(HappyTerminal (TokenNum _ happy_var_2))
	_
	 =  HappyAbsSyn4
		 (MyTokenAppend happy_var_2 happy_var_3
	)
happyReduction_2 _ _ _  = notHappyAtAll 

happyReduce_3 = happySpecReduce_2  4 happyReduction_3
happyReduction_3 (HappyTerminal (TokenNum _ happy_var_2))
	_
	 =  HappyAbsSyn4
		 (MyFinalTokenAppend happy_var_2
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TokenPrint _ -> cont 5;
	TokenAppend _ -> cont 6;
	TokenDuplicate _ -> cont 7;
	TokenLParen _ -> cont 8;
	TokenRParen _ -> cont 9;
	TokenNum _ happy_dollar_dollar -> cont 10;
	_ -> happyError' ((tk:tks), [])
	}
